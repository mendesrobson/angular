...
@Component({
    selector: 'app-consulta-movimentoconta',
    templateUrl: './consulta-movimentoconta.component.html'
})
export class ConsultaMovimentoContaComponent implements OnInit {
    ...
    public carregarMovimentoContaApropriacao : boolean = false;
    ...
    preencherFormMovimentoConta(movimentoConta: MovimentoConta): void {
        ...
        console.log("Preencher form movimento");
        console.log(movimentoConta);
        ...
        this.preencherMovimentoContaApropriacao();
       
        this.aPagar = this.movimentoConta.siglaDebitoCredito == "D" ? true : false;
    }
    ...
    remover(id : string) {
        this.router.navigate(['movimentoconta/excluir/'+id]);
    }

    preencherMovimentoContaApropriacao() {
        this.carregarMovimentoContaApropriacao = false;
        this.movimentoContaService.obterMovimentoContaApropriacao(this.movimentoContaId.toString())
        .subscribe(
            result => {
                console.log(result);

                for (var i = 0; i < result.length; i++) {
                    this.movimentoContaComponent.MovimentoConta.apropriacaoCentro.push(result[i]);
                }
                this.carregarMovimentoContaApropriacao = true;
