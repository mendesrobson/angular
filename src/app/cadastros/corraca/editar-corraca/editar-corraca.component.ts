...
@Component({
  selector: 'app-editar-corraca',
  templateUrl: './editar-corraca.component.html',
  styleUrls: []
})
export class EditarCorRacaComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    editarCorRaca() {
      if (this.corRacaForm.dirty && this.corRacaForm.valid) {
        const p = Object.assign({}, this.corRaca, this.corRacaForm.value);
  
        this.corRacaService.AtualizarCorRaca(p)
          .subscribe(
            result => {
              if (result) {
                  this.swal.showSwalSuccess('Cor/Raça atualizado com sucesso!');
    ...
    remover(id) {
      this.router.navigate(['corraca/excluir/' + id]);
    }
  
    reativar(id) {
      this.router.navigate(['corraca/reativar/' + id]);
    }
