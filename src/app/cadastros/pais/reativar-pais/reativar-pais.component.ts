...
@Component({
  selector: 'app-reativar-pais',
  templateUrl: './reativar-pais.component.html',
  styleUrls: []
})
export class ReativarPaisComponent implements OnInit, AfterViewInit {
  ...
  ngOnInit(): void {
    ...
    reativarPais() {
      this.paisService.ReativarPais(this.pais)
        .subscribe(
        result => {
          if (result) {
            this.swal.showSwalSuccess('País reativado com sucesso!');
          } else {
