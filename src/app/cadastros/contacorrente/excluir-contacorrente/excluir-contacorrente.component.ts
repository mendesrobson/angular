...
@Component({
  selector: 'app-excluir-contacorrente',
  templateUrl: './excluir-contacorrente.component.html',
  providers: [MaskService],
  styleUrls: []
})
export class ExcluirContaCorrenteComponent implements OnInit, AfterViewInit {
  ...
  cancelar() {
    if (this.contaCorrenteForm.controls['tipoContaId'].value == 35){
  ...
  removerContaCorrente() {
    this.contaCorrenteService.removerContaCorrente(this.contaCorrente)
      .subscribe(
      result => {
        this.swal.showSwalSuccess('Conta Corrente Removida com Sucesso');
