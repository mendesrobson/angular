...
@Component({
  selector: 'app-reativar-categoriacontareceber',
  templateUrl: './reativar-categoriacontareceber.component.html'
})
export class ReativarCategoriaContaReceberComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.swal.showSwalConfirm('Reativação', function (isConfirmed) {
      if (isConfirmed) {
        self.reativarCategoriaContaReceber();
      }
      else {
        self.cancelar();
      }
  ...
  reativarCategoriaContaReceber() {
    this.categoriaContaReceberService.reativarCategoriaContaReceber(this.categoriaContaReceber)
      .subscribe(
        result => {
          this.swal.showSwalSuccess('Categoria de Contas a Receber Reativada com Sucesso');
