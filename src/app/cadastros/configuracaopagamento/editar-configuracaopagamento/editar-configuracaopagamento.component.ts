...
@Component({
  selector: 'app-editar-configuracaopagamento',
  templateUrl: './editar-configuracaopagamento.component.html',
  providers: [MaskService],
  styleUrls: []
})

export class EditarConfiguracaopagamentoComponent implements OnInit, AfterViewInit {
  [x: string]: any;
  ...
  editarConfiguracaoPagamento() {
    if (this.configuracaoPagamentoForm.dirty && this.configuracaoPagamentoForm.valid) {
      ...
      this.configuracaoPagamentoService.AtualizarConfiguracaoPagamento(p)
        .subscribe(
        result => {
          this.swal.showSwalSuccess('Configuração Pagamento, Atualizado com Sucesso!');
  ...
  remover(id) {
    this.router.navigate(['configuracaopagamento/excluir/' + id]);
  }

  reativar(id) {
    this.router.navigate(['configuracaopagamento/reativar/' + id]);
  }
