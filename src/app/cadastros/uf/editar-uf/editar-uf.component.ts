...
export class EditarUfComponent implements OnInit, AfterViewInit {
    ...
    preencherFormUf(uf: Uf): void {
      ...
      !this.removerVisivel ? this.ufForm.disable() : this.ufForm.enable();
    ...
    editarUf() {
      if (this.ufForm.dirty && this.ufForm.valid) {
        ...
        this.ufService.AtualizarUf(p)
          .subscribe(
          result => {
            this.swal.showSwalSuccess('Uf, Atualizada com Sucesso');
    ...
    remover(id) {
      this.router.navigate(['uf/excluir/' + id]);
    }
  
    reativar(id) {
      this.router.navigate(['uf/reativar/' + id]);
    }
