...
@Component({
  selector: 'app-reativar-pessoa',
  templateUrl: './reativar-pessoa.component.html',
  styleUrls: ['./reativar-pessoa.component.css']
})
export class ReativarPessoaComponent implements OnInit {
  ...
  reativar() {
    this.pessoaService.ReativarPessoa(this.pessoa)
      .subscribe(
        () => {
          if (this.pessoaForm.controls['tipoId'].value == 1) {
            this.pessoaService.ReativarCliente(this.cliente)
              .subscribe(
                () => {
                  this.pessoaService.ReativarPessoa(this.pessoa).subscribe(() => {
                    this.swal.showSwalSuccess('Cliente Adicionado com Sucesso!');
                    this.router.navigate(['cliente/lista']);
                  },
                    (erro) => this.onError(erro)
                  );
                },
                error => {
                  this.onError(error)
          ...
          if (this.pessoaForm.controls['tipoId'].value == 2) {
            ...
            this.pessoaService.ReativarFornecedor(this.fornecedor)
              .subscribe(
                () => {
                  this.pessoaService.ReativarPessoa(this.pessoa).subscribe(() => {
          ...
          if (this.pessoaForm.controls['tipoId'].value == 3) {
            this.pessoaService.ReativarEmpresa(this.empresa)
              .subscribe(
                () => {
                  this.pessoaService.ReativarPessoa(this.pessoa).subscribe(() => {
