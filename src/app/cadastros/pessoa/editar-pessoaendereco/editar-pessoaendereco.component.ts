...
export class EditarPessoaenderecoComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.preencherEnderecoForm(this.pessoaComponent.PessoaEndereco);        
  }

  preencherEnderecoForm(endereco: Endereco): void {
  ...
  editarEndereco() {
    ...
    if (this.enderecoForm.dirty && this.enderecoForm.valid) {
      ...
      if (this.pessoaId > 0) {
        p.pessoaId = this.pessoaId;
        this.pessoaService.AtualizarPessoaEndereco(p)
          .subscribe(
            result => {              
              for (let i = 0; this.pessoaComponent.Pessoa.endereco.length > i; i++) {
                if (result.id == this.pessoaComponent.Pessoa.endereco[i].id) {
                  this.pessoaComponent.Pessoa.endereco[i] = result;
                }
              }
              this.listPessoaEndereco.pessoaEnderecoGravado('Endereço editado com sucesso!');
              this.close();
            },
            error => {
              this.errors;
        ...
        for (let i = 0; this.pessoaComponent.Pessoa.endereco.length > i; i++) {
          if (p.id == this.pessoaComponent.Pessoa.endereco[i].id) {
            this.pessoaComponent.Pessoa.endereco[i] = p;
          }
        }
        this.listPessoaEndereco.pessoaEnderecoGravado('Endereço editado com sucesso!');
        this.close();
