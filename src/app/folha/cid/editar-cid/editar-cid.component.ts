...
@Component({
  selector: 'app-editar-cid',
  templateUrl: './editar-cid.component.html',
  styleUrls: []
})
export class EditarCidComponent implements OnInit, AfterViewInit {
  ...
  obterCid(id: string) {
    this.cidService.obterCid(id)
      .subscribe(cid => this.preencherFormCid(cid),
        response => {
  ...
  preencherFormCid(cid: Cid): void {
    ...
    !this.removerVisivel ? this.cidForm.disable() : this.cidForm.enable();
  ...
  editarCid() {
    if (this.cidForm.dirty && this.cidForm.valid) {
      ...
      this.cidService.AtualizarCid(p)
        .subscribe(
          result => {
            if (result) {
              this.swal.showSwalSuccess('Cid, Atualizado com Sucesso!');
  ...
  remover(id) {
    this.router.navigate(['cid/excluir/' + id]);
  }

  reativar(id) {
    this.router.navigate(['cid/reativar/' + id]);
  }
