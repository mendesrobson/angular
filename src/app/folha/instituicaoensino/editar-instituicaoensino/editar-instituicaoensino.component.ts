...
@Component({
  selector: 'app-editar-instituicaoensino',
  templateUrl: './editar-instituicaoensino.component.html',
  styleUrls: []
})
export class EditarInstituicaoensinoComponent implements OnInit, AfterViewInit {
 ...
 constructor(
  ...
  preencherFormInstituicaoEnsino(instituicaoEnsino: InstituicaoEnsino): void {
    ...
    !this.removerVisivel ? this.instituicaoEnsinoForm.disable() : this.instituicaoEnsinoForm.enable();
  ...
  editarInstituicaoEnsino() {
    if (this.instituicaoEnsinoForm.dirty && this.instituicaoEnsinoForm.valid) {
      ...
      this.instituicaoEnsinoService.AtualizarInstituicaoEnsino(p)
        .subscribe(
          result => {
            if (result) {
              this.swal.showSwalSuccess('Instituição de Ensino, Atualizado com Sucesso!');
