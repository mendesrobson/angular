...
@Component({
  selector: 'app-reativar-tipoafastamento',
  templateUrl: './reativar-tipoafastamento.component.html',
  styleUrls: []
})
export class ReativarTipoafastamentoComponent implements OnInit, AfterViewInit {
  ...
  reativarTipoAfastamento() {
    this.tipoAfastamentoService.reativarTipoAfastamento(this.tipoAfastamento)
      .subscribe(
      result => {
        if (result) {
          this.swal.showSwalSuccess('Tipo de Afastamento reativado com sucesso!');
        } else {
  ...
  cancelar() {
    this.router.navigate(['tipoafastamento/editar/' + this.tipoAfastamentoId]);
  }
