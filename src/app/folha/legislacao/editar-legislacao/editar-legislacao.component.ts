...
@Component({
  selector: 'app-editar-legislacao',
  templateUrl: './editar-legislacao.component.html',
  styleUrls: []
})
export class EditarLegislacaoComponent implements OnInit , AfterViewInit {
  ...
  constructor(
   ...
   preencherFormLegislacao(legislacao: Legislacao): void {
     ...
     !this.removerVisivel ? this.legislacaoForm.disable() : this.legislacaoForm.enable();
   ...
   editarLegislacao() {
     if (this.legislacaoForm.dirty && this.legislacaoForm.valid) {
       ...
       this.legislacaoService.AtualizarLegislacao(p)
         .subscribe(
           result => {
             if (result) {
