...
@Component({
  selector: 'app-reativar-partecorpoatingida',
  templateUrl: './reativar-partecorpoatingida.component.html',
  styleUrls: []
})
export class ReativarPartecorpoatingidaComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.swal.showSwalConfirm('Reativação', function (isConfirmed) {
      if (isConfirmed) {
        self.reativarparteCorpoAtingida();
      } else {
        self.cancelar();
      }
    });
  ...
  reativarparteCorpoAtingida() {
    this.parteCorpoAtingidaService.reativarParteCorpoAtingida(this.parteCorpoAtingida)
      .subscribe(
        result => {
          if (result) {
            this.swal.showSwalSuccess('Parte Corpo Atiginda, reativado com sucesso!');
          } else {
