...
@Component({
  selector: 'app-reativar-exposicaoagentenocivo',
  templateUrl: './reativar-exposicaoagentenocivo.component.html',
  styleUrls: []
})
export class ReativarExposicaoagentenocivoComponent implements OnInit, AfterViewInit {
  ...
  ngAfterViewInit(): void {
    ...
    this.swal.showSwalConfirm('Reativação', function (isConfirmed) {
      if (isConfirmed) {
        self.reativarexposicaoAgenteNocivo();
      } else {
        self.cancelar();
      }
    });
  ...
  reativarexposicaoAgenteNocivo() {
    this.exposicaoAgenteNocivoService.reativarExposicaoAgenteNocivo(this.exposicaoAgenteNocivo)
      .subscribe(
        result => {
          if (result) {
            this.swal.showSwalSuccess('Exposição Agente Nocivo, reativado com sucesso!');
          } else {
