...
@Component({
  selector: 'app-reativar-cargo',
  templateUrl: './reativar-cargo.component.html',
  styleUrls: []
})

export class ReativarCargoComponent implements OnInit , AfterViewInit {
  [x: string]: any;
  ...
  reativarCargo() {
    this.cargoService.reativarCargo(this.cargo)
      .subscribe(
      result => {
        if (result) {
          this.swal.showSwalSuccess('Cargo Reativado com Sucesso!');
        } else {
