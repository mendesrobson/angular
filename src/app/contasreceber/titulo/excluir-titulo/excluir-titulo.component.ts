...
export class ExcluirTituloComponent implements OnInit, AfterViewInit {
  [x: string]: any;
  ...
  removerTitulo() {
    ...
    this.tituloService.RemoverTitulo(this.titulo)
      .subscribe(
      result => {
        if(result['valid']){

              this.swal.showSwalSuccess('Titulo, Removido com Sucesso');
              if (this.titulo.naturezaId === 1) {
          ...
          this.swal.showSwalErro(result['message']);

          if(this.titulo.naturezaId === 1){
            let url = location.hash.replace("#/", "").replace("excluir", "editar/receber");
            this.router.navigate([url]);
          }
          else if(this.titulo.naturezaId === 2){
            let url = location.hash.replace("#/", "").replace("excluir", "editar/pagar");
            this.router.navigate([url]);
          }
